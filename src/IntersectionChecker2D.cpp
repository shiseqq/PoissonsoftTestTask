...
bool IntersectionChecker2D::doLinesIntersect(const Point2D& p1, const Point2D& q1,
                                             const Point2D& p2, const Point2D& q2) const {
...
int IntersectionChecker2D::orientation(const Point2D& p, const Point2D& q, const Point2D& r) const {
    float val = (q.y - p.y) * (r.x - q.x) - (q.x - p.x) * (r.y - q.y);
    if (val == 0) return 0;
    return (val > 0) ? 1 : 2;
}

bool IntersectionChecker2D::onSegment(const Point2D& p, const Point2D& q, const Point2D& r) const {
...
bool IntersectionChecker2D::isPointInsideTriangle(const Point2D& p, const Triangle& t) const {
...
bool IntersectionChecker2D::checkIntersection(const Triangle& t1, const Triangle& t2) const {
    for (int i = 0; i < 3; ++i) {
